...
export default function AdminAttendance() {
  ...
  const [form, setForm] = useState({ registration_id: '', batch_id: '', session_date: '', session_title: '', status: 'present' });
  const [bulkForm, setBulkForm] = useState({ batch_id: '', session_date: '', session_title: '', status: 'present' });
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const record = await appClient.entities.AttendanceRecord.create({
        ...
        session_date: data.session_date,
        status: data.status,
        join_time: data.status === 'present' || data.status === 'late' ? '09:00' : '-',
        leave_time: data.status === 'present' || data.status === 'late' ? '12:00' : '-',
      });
