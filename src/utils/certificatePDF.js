...
export async function generateCertificatePDF(cert) {
  ...
  if (!logoEmbedded) {
    ...
    pdf.text('CENTER OF EXCELLENCE', logoBoxX + logoBoxW / 2, logoBoxY + 14, { align: 'center' });
  }
  ...
  pdf.setFontSize(10);
  pdf.setFont('helvetica', 'bold');
  pdf.text('V-TEKI CoE', s2X, sigY + 6, { align: 'center' });
  pdf.setFontSize(8);
  pdf.setFont('helvetica', 'normal');
  pdf.setTextColor(100, 100, 100);
  ...
  pdf.setFont('helvetica', 'normal');
  pdf.text(
    `Verifikasi Sertifikat di vteki.academy/verify | Berlaku hingga ${format(new Date(new Date().setFullYear(new Date().getFullYear() + 3)), 'dd MMM yyyy')}`,
    W / 2,
    H - 15,
    { align: 'center' }
  );
  ...
  // ════════════════════════════════════════════════════════════════
  try {
    const verifyUrl = `https://vteki.academy/verify?id=${cert.certificate_number || 'demo'}`;
