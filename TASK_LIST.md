# Task List

Last updated: 2026-06-10

## Done

- [x] Local demo auth for all roles
- [x] Supabase disabled by default
- [x] Public navbar fixed on mobile
- [x] Participant dashboard and program progress visible
- [x] Participant attendance summary visible
- [x] Participant attendance detail cleanup
- [x] Certificate eligibility logic in place
- [x] Certificate PDF generation wired
- [x] Trainer identity resolution fixed
- [x] Trainer assessment review flow polish
- [x] Trainer feedback page polish
- [x] Trainer reports page polish
- [x] Trainer batches list and detail dialog polish
- [x] Corporate dashboard clarity and empty-state polish
- [x] Admin invoice creation flow
- [x] Participant certificates page end-to-end
- [x] Admin certificate generation end-to-end
- [x] README updated
- [x] Vite dev config adjusted for stable local startup

## In Progress

- [ ] Bundle size reduction and code splitting

## Next Up

- [ ] Verify trainer dashboard data for all seeded trainers
- [ ] Verify trainer batches list and detail dialog
- [ ] Verify admin attendance participant selection end-to-end
- [ ] Improve demo data coverage for edge cases
- [ ] Add more explicit empty states where needed

## Later

- [ ] Revisit Supabase integration when the MVP demo flow is stable
- [ ] Add RLS and backend schema only if the project moves back to Supabase mode
- [ ] Review whether the Vite app should be migrated to a full-stack framework later
